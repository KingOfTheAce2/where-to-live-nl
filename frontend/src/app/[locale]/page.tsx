          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            {snapshot && snapshot.snapshot.woz && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">WOZ property value</h3>
                  {snapshot.snapshot.woz.available ? (
                    <span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-1 rounded">Available</span>
                  ) : (
                    <span className="text-xs font-medium text-gray-500 bg-gray-50 px-2 py-1 rounded">Limited Data</span>
                  )}
                </div>
                ...
                {snapshot.snapshot.woz.available && snapshot.snapshot.woz.value ? (
                  <div className="space-y-3">
                    <div className="flex justify-between items-baseline">
                      <span className="text-sm text-gray-600">2024 Value</span>
                      ...
                      <div className="pt-2 border-t border-gray-100">
                        <details className="text-xs">
                          <summary className="cursor-pointer text-blue-600 hover:text-blue-800">
                            View historical values (2014-2024)
                          </summary>
                  ...
                  <div className="text-xs text-gray-600 space-y-3">
                    <p className="leading-relaxed">{snapshot.snapshot.woz.note}</p>
                    <a
                      href="https://www.wozwaardeloket.nl/"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 transition-colors font-medium"
                    >
                      🔍 Look up WOZ value on wozwaardeloket.nl
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                      </svg>
                    </a>
                    <p className="text-gray-500 italic text-[11px]">
                      We're gradually collecting WOZ data. Ingestion is slow (0.5 req/sec) to respect API limits.
                    </p>
                  </div>
                )}
              </div>
            ...
            {snapshot?.snapshot.energy_consumption && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">⚡ average utility costs</div>
                ...
                {snapshot.snapshot.energy_consumption.estimated_annual_cost_eur ? (
                  <>
                    ...
                    <div className="bg-blue-50 rounded-lg p-3 mb-3">
                      <div className="flex items-center justify-center gap-3">
                        <div className="text-2xl font-bold text-blue-600">
                          €{snapshot.snapshot.energy_consumption.monthly_cost_eur}/month
                        </div>
                      ...
                      <div className="text-xs text-gray-600 mt-2 text-center">
                        €{snapshot.snapshot.energy_consumption.estimated_annual_cost_eur}/year total
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {snapshot.snapshot.energy_consumption.cost_breakdown?.gas_eur && (
                        <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                          <div>
                            <div className="text-gray-700">🔥 Gas</div>
                            <div className="text-xs text-gray-500">
                      ...
                      {snapshot.snapshot.energy_consumption.cost_breakdown?.electricity_eur && (
                        <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                          <div>
                            <div className="text-gray-700">⚡ Electricity</div>
                            <div className="text-xs text-gray-500">
                    ...
                    {snapshot.snapshot.energy_consumption.district_heating_percentage &&
                     snapshot.snapshot.energy_consumption.district_heating_percentage > 0 && (
                      <div className="mt-3 pt-2 border-t border-gray-100 text-xs text-gray-600">
                        ℹ️ {snapshot.snapshot.energy_consumption.district_heating_percentage}% of homes here use district heating (stadsverwarming)
                      </div>
                    )}
                    ...
                    <details className="mt-3 pt-2 border-t border-gray-100 text-xs">
                      <summary className="cursor-pointer text-gray-600 hover:text-gray-900">
                        ℹ️ About these costs
                      </summary>
                      <div className="mt-2 space-y-1 text-gray-600">
                        <p>
                          <strong>Data source:</strong> CBS 2024 neighborhood averages
                        </p>
                        <p>
                          <strong>Prices (Jan 2025):</strong>
                        </p>
                        <ul className="list-disc list-inside ml-2 space-y-1">
                          <li>Gas: €1.35/m³</li>
                          <li>Electricity: €0.34/kWh</li>
                          <li>Water: €2.61/m³ (incl. taxes)</li>
                        </ul>
                        <p className="mt-2">
                          <strong>Note:</strong> Actual costs vary by:
                        </p>
                        <ul className="list-disc list-inside ml-2">
                          <li>Home size & insulation</li>
                          <li>Number of residents</li>
                          <li>Personal usage habits</li>
                          <li>Energy provider & contract</li>
                        </ul>
                      </div>
                    </details>
                  </>
                  ...
                  <div className="text-sm text-gray-500 text-center py-4">
                    No energy consumption data available for this area
                  </div>
                )}
              </div>
            )}
            ...
            {snapshot?.snapshot.proximity && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">📏 distance to facilities (CBS 2024)</div>
                <div className="space-y-4 text-sm">
                  {/* Transport */}
                  {(snapshot.snapshot.proximity.dist_to_train_station_km || snapshot.snapshot.proximity.dist_to_highway_onramp_km) && (
                    <div>
                      <div className="text-xs font-medium text-gray-700 mb-2">🚆 Transport</div>
                      <div className="space-y-1">
                        ...
                        {snapshot.snapshot.proximity.dist_to_major_train_station_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Major train station</span>
                        ...
                        {snapshot.snapshot.proximity.dist_to_highway_onramp_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Highway on-ramp</span>
                  ...
                  {(snapshot.snapshot.proximity.dist_to_supermarket_km || snapshot.snapshot.proximity.dist_to_daily_groceries_km) && (
                    <div className="pt-2 border-t border-gray-100">
                      <div className="text-xs font-medium text-gray-700 mb-2">🛒 Shopping</div>
                      <div className="space-y-1">
                        {snapshot.snapshot.proximity.dist_to_supermarket_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Large supermarket</span>
                            ...
                            {snapshot.snapshot.proximity.supermarket_within_1km !== undefined && (
                              <span className="text-gray-500 ml-2">({snapshot.snapshot.proximity.supermarket_within_1km.toFixed(1)} within 1km)</span>
                            )}
                          </div>
                        )}
                        {snapshot.snapshot.proximity.dist_to_daily_groceries_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Daily groceries</span>
                  ...
                  {(snapshot.snapshot.proximity.dist_to_gp_km || snapshot.snapshot.proximity.dist_to_hospital_km) && (
                    <div className="pt-2 border-t border-gray-100">
                      <div className="text-xs font-medium text-gray-700 mb-2">🏥 Healthcare</div>
                      <div className="space-y-1">
                        {snapshot.snapshot.proximity.dist_to_gp_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">GP (huisarts)</span>
                            ...
                            {snapshot.snapshot.proximity.gp_within_1km !== undefined && (
                              <span className="text-gray-500 ml-2">({snapshot.snapshot.proximity.gp_within_1km.toFixed(1)} within 1km)</span>
                            )}
                          </div>
                        )}
                  ...
                  {(snapshot.snapshot.proximity.dist_to_primary_school_km || snapshot.snapshot.proximity.dist_to_daycare_km) && (
                    <div className="pt-2 border-t border-gray-100">
                      <div className="text-xs font-medium text-gray-700 mb-2">🎓 Education & Childcare</div>
                      <div className="space-y-1">
                        ...
                        {snapshot.snapshot.proximity.dist_to_daycare_km && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Daycare</span>
                  ...
                  {(snapshot.snapshot.proximity.dist_to_library_km || snapshot.snapshot.proximity.dist_to_museum_km || snapshot.snapshot.proximity.dist_to_cinema_km) && (
                    <div className="pt-2 border-t border-gray-100">
                      <div className="text-xs font-medium text-gray-700 mb-2">🎭 Culture & Recreation</div>
                      <div className="space-y-1">
                ...
                <div className="text-xs text-gray-500 mt-3 pt-2 border-t border-gray-100">
                  Official CBS proximity metrics for {snapshot.snapshot.proximity.area_name}
                </div>
              </div>
            )}
            ...
            {snapshot?.snapshot.emergency_services && snapshot.snapshot.emergency_services.count > 0 && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">
                  🚨 emergency services nearby ({snapshot.snapshot.emergency_services.count})
                </div>
                <div className="space-y-2">
            ...
            {snapshot?.snapshot.cultural_amenities && snapshot.snapshot.cultural_amenities.count > 0 && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">
                  🎭 cultural amenities nearby ({snapshot.snapshot.cultural_amenities.count})
                </div>
                <div className="space-y-2">
            ...
            {snapshot?.snapshot.healthcare_facilities && snapshot.snapshot.healthcare_facilities.count > 0 && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">
                  🏥 healthcare facilities ({snapshot.snapshot.healthcare_facilities.count})
                </div>
                <div className="space-y-2">
                ...
                <div className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-100">
                  Includes hospitals, GPs, pharmacies, dentists, and specialists
                </div>
              </div>
            )}
            ...
            {/* WWS Rent Calculator */}
            <WWSCalculator
              initialSqm={75}  // Default estimate
              initialEnergyLabel="C"  // Average Dutch home
            ...
            {snapshot?.snapshot.demographics?.municipality && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="text-sm font-medium text-gray-900 mb-3">📍 local government</div>
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                  ...
                  <div className="space-y-2">
                    <p className="text-xs text-gray-600 font-medium">Quick Links:</p>
                    <div className="flex flex-col gap-2">
                      ...
                      <a
                        href={`https://www.google.com/search?q=${encodeURIComponent(
                          snapshot.snapshot.demographics.municipality + ' afvalkalender trash collection'
                        )}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-xs text-gray-600 hover:text-gray-700 hover:underline"
                      >
                        🗑️ Trash collection schedule
                      </a>
                      <a
                        href={`https://www.google.com/search?q=${encodeURIComponent(
                          snapshot.snapshot.demographics.municipality + ' parkeervergunning parking permit'
                        )}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-xs text-gray-600 hover:text-gray-700 hover:underline"
                  ...
                  <div className="bg-blue-50 rounded p-3 text-xs text-blue-800 border border-blue-100">
                    <p><strong>💡 Tip for Expats:</strong> Most gemeente services are available online. You can arrange permits, report issues, and access local information through their website.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="text-sm font-medium text-blue-900 mb-2">
                Optional: Add work address
              </div>
              <div className="text-xs text-blue-700 mb-3">
                Calculate travel time from this property to your workplace. Routes are cached in a database to make repeated queries faster and minimize API costs.
              </div>
              ...
              {selectedWorkAddress && travelTime && (
                <div className="space-y-3">
                  <div className="bg-white rounded-md p-3 border border-blue-200">
                    <div className="text-xs font-medium text-gray-700 mb-1">Work address:</div>
                    <div className="text-sm text-gray-900">{selectedWorkAddress.address}</div>
                  </div>

                  <div className="text-xs font-medium text-blue-900 mb-2">Estimated commute time:</div>
                    ...
                    {travelTime.byBike && (
                      <div className="flex items-center justify-between bg-white rounded-md p-2 border border-blue-100">
                        <span className="flex items-center gap-2">
                          <span>🚴</span>
                          <span className="text-gray-700">By bike</span>
                        </span>
                  ...
                  <div className="text-xs text-blue-600 mt-2">
                    ℹ️ Actual route times via OpenRouteService
                  </div>
